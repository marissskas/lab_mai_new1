...
class TestMemoryDatabase(unittest.TestCase):
    ...
    def test_sort_records_ascending(self):
        ...
        self.db.create_table("студенты", ("id", "имя"))
        self.db.insert_record("студенты", {"id": "3", "имя": "Вера"})
        self.db.insert_record("студенты", {"id": "1", "имя": "Анна"})
        self.db.insert_record("студенты", {"id": "2", "имя": "Борис"})
    ...
    def test_sort_records_numeric_field_as_string(self):
        ...
        self.db.create_table("products", ("id", "price"))
        self.db.insert_record("products", {"id": "1", "price": "100"})
        self.db.insert_record("products", {"id": "2", "price": "20"})
        self.db.insert_record("products", {"id": "3", "price": "300"})
        
        # Сортировка по возрастанию - должна быть числовая (20, 100, 300), 
        # а не лексикографическая (100, 20, 300)
        sorted_records = self.db.sort_records("products", "price", reverse=False)
        result_prices = [r["price"] for r in sorted_records]
        expected_prices = ["20", "100", "300"]
        self.assertEqual(result_prices, expected_prices)
        
        # Сортировка по убыванию
        sorted_records_desc = self.db.sort_records("products", "price", reverse=True)
        result_prices_desc = [r["price"] for r in sorted_records_desc]
        expected_prices_desc = ["300", "100", "20"]
        self.assertEqual(result_prices_desc, expected_prices_desc)
